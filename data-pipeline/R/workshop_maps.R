# data-pipeline/R/workshop_maps.R
#
# A3 print sheets for Living Lab workshops: one map per Living Lab, built from the report's
# cover-page locator (maps_vector.R's `ll_map_locator()` main panel -- same padded extent,
# same smoothed halo around the boundary, same veil over the ring between boundary and halo
# edge, same boundary outline) without the Germany overview panel, and with two swaps:
#
#  * Basemap. The satellite imagery is replaced by an OpenStreetMap basemap drawn here from
#    vector data -- landuse, water, railways and the road network down to a scale-dependent
#    class -- so it prints crisply at A3 instead of as enlarged screen tiles. The data is a
#    regional extract of the daily Protomaps OSM planet build (`pmtiles extract`, the same
#    CLI the pipeline already uses), cached under data/_cache/workshop_osm/.
...
#
# Usage (run from data-pipeline/R so renv activates; needs the `pmtiles` CLI on PATH or
...
# --- Own-file resolution + load the report modules ---------------------------------------
...
source(normalizePath(file.path(.wm_dir, "report", "maps_vector.R"), winslash = "/", mustWork = TRUE))
...
WM_COLOURS <- list(
  ...
  forest = "#d3e5c3",
  builtup = "#e6dfd8",
  water = ll_tokens()$palettes$soil$waterFill,
  water_line = ll_tokens()$palettes$soil$waterStroke,
  rail = "#707070",
  road_casing = "#9c9c9c",
  road = "#ffffff",
  primary = "#f7dc8a",
...
wm_osm_extract <- function(slug, bbox_ll, refresh = FALSE) {
  cache_dir <- file.path(ll_repo_root(), "data", "_cache", "workshop_osm")
  dir.create(cache_dir, recursive = TRUE, showWarnings = FALSE)
  path <- file.path(cache_dir, paste0(slug, ".pmtiles"))
...
wm_map_plot <- function(slug, layers, halo, boundary_3857, labels, layout, font_family) {
  ...
  # Everything outside the halo is painted out, which reproduces the report locator's clipped
  # imagery without clipping every basemap feature individually.
  ...
  ggplot2::ggplot() +
    ...
    ggplot2::geom_sf(data = boundary_3857, fill = NA, colour = brand$outlineColor, linewidth = WM_BOUNDARY_LW) +
    ggplot2::geom_point(
...
wm_draw_sheet <- function(sheet) {
  ...
  km_label <- function(v) ll_format_number(v, sheet$lang)
...
wm_build_sheet <- function(slug, lang, refresh_osm) {
  ...
  # The report locator's own extent and halo, with one change: holes in the halo are filled.
  # A Living Lab can surround a city it excludes (north-hessian-loess around Kassel), and on a
  # workshop map that city is the landmark people orient by -- it gets the veiled basemap
  # like the rest of the surroundings instead of a blank.
  ...
  halo$halo <- .wm_fill_holes(halo$halo)
  halo$ring <- sf::st_make_valid(sf::st_difference(halo$halo, core))
  ...
  list(
    ...
    brand = ll_brand(slug),
    scale_label = ll_format_number(layout$scale_denominator, lang),
...
wm_write_readme <- function(out_dir) {
  ...
  lines <- c(
    "LL-Explorer workshop maps -- A3 print sheets",
    "=============================================",
    "",
    "PRINTING",
...
wm_main <- function(args = commandArgs(trailingOnly = TRUE)) {
  ...
  out_dir <- opt("out", file.path(ll_repo_root(), "workshop_maps_a3"))
  ...
  for (slug in slugs) {
    ll_lab(slug) # fails fast with the list of known slugs
